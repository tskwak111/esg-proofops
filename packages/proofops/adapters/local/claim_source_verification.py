...
def claim_source_policy():
    return dict(
        schema="claim_source_policy_v2",
        claim_validator_sha256=sha256(Path(claim_validation.__file__).read_bytes()).hexdigest(),
...
def _unique_quote(text, quote):
    ...
    try:
        claim_validation.validate_extraction_response(
            {
                "spans": [
                    dict(
                        char_start=start,
        ...
        return False
    return not (end < len(text) and quote[-1] in "%‰" and text[end].isalnum())
...
def claim_source_reader(policy):
    """Known version dispatch; never load verifier code named by an input artifact."""
    import sys
