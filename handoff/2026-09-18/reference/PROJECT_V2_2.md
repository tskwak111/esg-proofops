...
> **"이 문장이 하는 주장을, 이 기업의 공시 패키지 안에서 확인할 수 있는가?"**

**검증 단위는 문서가 아니라 공시 패키지다.** 지속가능경영보고서(또는 사업보고서 지속가능성 정보란)와 함께, 같은 기업이 같은 기간에 낸 **재무제표·사업보고서**를 대조 대상에 포함한다.

이것은 기능 추가가 아니라 기준 요구사항이다. IFRS S1 문단 21은 기업이 지속가능성 관련 재무공시와 재무제표를 포함한 다른 일반목적재무보고서 간의 연계를 이용자가 이해할 수 있도록 정보를 제공할 것을 요구한다(세부는 문단 B39~B44). 즉 **연계는 우리가 덧붙이는 기능이 아니라 기준이 이미 요구하는 공시 항목**이며, v2.0 루브릭에는 이 항목이 통째로 빠져 있었다.

다만 대조 대상은 어디까지나 **같은 기업이 스스로 공시한 자료**로 한정한다. 뉴스·NGO 보도·제3자 배출량 DB 등 외부 데이터로 성과의 진위를 검증하는 것은 여전히 범위 밖이다(3.5절과 동일한 원칙).
...
6. **연계(reconciliation) 현황** — 지속가능성 정보와 재무제표 간 차이 항목, 그리고 그 차이를 설명하는 문구가 공시에 존재하는지
7. **감사 추적 로그** — 판정별 근거 위치, 사용 규칙 버전, 검토자 확정 이력
8. **기준별 충족 현황** — KSSB·K-ESG 등 기준별, 산업 중대성을 반영한 충족률
...
### 2.8 재무제표와 사업보고서 (연계 검증용 기초)

**연결범위** — 재무제표는 지배기업과 종속기업을 하나로 묶어 작성한다. 어느 회사가 종속기업인지는 주석의 종속기업 목록에 나온다. 배출량 산정의 조직경계(2.5절)와 **다를 수 있고, 다른 것이 정상인 경우가 많다.** 회계는 지배력 기준, 배출량은 운영통제 기준을 쓰는 것이 대표적이다.

**주석(footnotes)** — 재무제표 본문 숫자의 근거와 세부 내역. 종속기업 목록, 부문별 정보, 자본적지출 약정, 충당부채, 우발부채가 여기 있다. 연계 검증에 실제로 쓰이는 정보는 대부분 본문이 아니라 주석에 있다.

**부문별 정보(segment)** — 사업 부문별 매출·이익. "친환경 제품 매출 비중" 주장을 대조할 때 쓴다.

**자본적지출(CAPEX)과 약정** — 현금흐름표의 투자활동과 주석의 약정 사항. "2030년까지 O조원 투자" 같은 정량 약속을 대조할 때 쓴다.

**회계감사기준 720(기타정보)** — 감사인은 감사보고서에 포함되지 않은 기타정보를 읽고 재무제표와의 중대한 불일치 여부를 확인할 책임이 있다. 2028년부터 지속가능성 정보가 사업보고서에 실리면 이 책임의 대상이 된다. **회계법인·인증기관 고객의 수요가 여기서 나온다.**

**DART** — 금융감독원 전자공시시스템. 사업보고서와 재무제표를 **구조화된 형태로 조회**할 수 있다. PDF 파싱이 아니라 API 조회다(6.1절).
...
| **연계·회계** | 공시와 재무제표가 정합하는가 | IFRS S1 문단 21·B39~B44, K-IFRS 1110·1036·1037, 회계감사기준 720 |
...
| 기준 | 시스템에서의 역할 |
|---|---|
| **IFRS S1 문단 21, B39~B44** | 지속가능성 공시와 재무제표 간 연계 정보 요구. **C군 루브릭 전체의 근거** |
| **K-IFRS 1110** 연결재무제표 | 연결범위 판단 기준 → C1 대조 |
| **K-IFRS 1036** 자산손상 | 기후 전환 리스크의 재무영향 → C5 대조 (2단계) |
| **K-IFRS 1037** 충당부채·우발부채 | 환경 복구·정화 책임 → C5 대조 (2단계) |
| **회계감사기준 720** 기타정보 | 감사인의 기타정보 검토 책임. **회계법인 고객 수요의 근거이자 제품 포지셔닝의 법적 배경** |
| 자본시장법 사업보고서 기재사항 | 2028년 이후 입력 문서의 법적 근거 |
...
기존 G·P·M 군은 건드리지 않고 별도 군으로 추가한다. E0~E3 사다리와 라벨 산출에는 관여하지 않으며 **별도 축으로 출력**한다(4.10절).

| # | 요소 | 대조 대상 | 발동 조건 | 단계 |
|---|---|---|---|---|
| C1 | 연결범위 일치 | 배출량 조직경계 ↔ 종속기업 목록 주석 | 조직경계를 언급하는 주장 | 1 |
| C2 | 보고기간 일치 | 지속가능성 정보 기준일 ↔ 결산일 | 정량 성과 주장 | 1 |
| C3 | 정량 약속의 재무 근거 | 투자 약속 금액 ↔ 현금흐름표·자본적지출 약정 주석 | 금액을 명시한 투자 약속 | 1 |
| C4 | 분류 기준 공시 | 친환경 매출 비중 ↔ 부문별 정보 | 매출·비중 주장 | 1 |
| C5 | 재무영향 서술 정합 | 전환 리스크 중대성 서술 ↔ 손상·충당부채 주석 | 재무영향 언급 | **2** |

> **C5는 1단계에 넣지 않는다.** 회계 판단이 필요해 오탐 위험이 크다. C1·C3만으로도 제품 성격이 바뀐다(12.1절).
...
### 4.10 연계 판정의 출력 규칙 (신설 · 가장 중요)

**불일치는 결손이 아니다.** 재무제표는 K-IFRS 연결 기준을, 배출량은 GHG Protocol 운영통제 기준을 쓴다. 목적과 기준이 다르므로 숫자가 달라야 맞는 경우가 많다. 이를 결손으로 처리하면 오탐이 쏟아지고 기업 고객은 도구를 버린다.

따라서 두 가지를 지킨다.

**(1) 연계 판정 결과는 label·evidence_grade에 반영하지 않는다.** 별도 필드로만 출력한다.

**(2) 판정하는 것은 "값이 같은가"가 아니라 "다르다면 그 차이를 설명하는 문구가 공시에 있는가"이다.** 이것이 IFRS S1이 요구하는 연계 정보의 실질이며, 오탐을 막는 유일한 방법이다.
...
"reconciliation": [{
  ...
  "basis": {"standard": "IFRS S1", "clause": "21", "text": "재무제표와의 연계 정보 제공"}
}]
```

`status`는 `matched` / `needs_explanation` / `not_applicable` 세 값만 가진다. **`mismatch`나 `error` 같은 값을 두지 않는다.** 값 이름 하나가 제품의 성격을 규정한다.

**면책 문구 (제품 UI와 계약서에 필수)**: 본 기능은 회계 처리의 적정성을 판단하지 않으며, 공시 간 차이에 대한 설명의 존재 여부만 점검한다.

**회계 판단 금지 경계선**

팀에 회계 도메인 전문가가 없다는 것이 이 기능의 설계 제약이다. 경계선은 권고가 아니라 구현 금지 사항이다.

| 해도 되는 것 | 하면 안 되는 것 |
|---|---|
| 두 문서의 값 비교 | 인식·금액·분류의 **적정성** 평가 |
| 항목이 문서에 **존재하는지** 확인 | 연결범위 판단의 적정성 평가 |
| 차이를 **설명하는 문구**가 있는지 탐색 | 추정·가정의 합리성 평가 |
| 분류 **기준이 공시되었는지** 확인 | 재무 영향의 추정 |

**판별 질문**: 산출물에 "~해야 한다", "~가 누락되었다", "~이 부적절하다"가 들어가면 회계 판단이다. 우리 산출물은 **"~에 대한 설명이 확인되지 않았다"**까지다.

구현 안전장치 3종(stage 런타임 가드, 금지 어휘 필터, 경계선 회귀 테스트)은 「연계 검증 모듈 명세」 1.3절 참조.
...
입력이 두 종류이며 처리 방식이 근본적으로 다르다.

| 경로 | 대상 | 처리 |
|---|---|---|
| **A. 비구조화** | 지속가능경영보고서 PDF, 사업보고서 지속가능성 정보란 | 레이아웃 인식 파싱 (1-2) |
| **B. 구조화** | 재무제표, 주석, 종속기업 목록, 부문별 정보, 현금흐름표, 자본적지출 약정 | **DART Open API 조회** — 파싱 불필요 |

경로 B는 항목 코드로 조회하므로 PDF 파싱보다 난이도가 낮고 정확도가 높다. **연계 검증의 병목은 재무 데이터가 아니라 지속가능성 문서 쪽이다.**

**대조 기간 결정 규칙 (필수)**

두 문서는 **발간월이 다를 뿐 보고대상기간은 같다.** 2025년 3월 사업보고서와 2025년 6~7월 지속가능경영보고서는 모두 FY2024를 보고한다. 따라서 **동일 회계연도로 대조하며, 발간 연도로 맞추지 않는다.**

발간 순서는 검증의 정당성을 강화한다. 사업보고서가 먼저 확정되므로, 지속가능경영보고서 작성 시점에 기업은 이미 확정된 재무 데이터를 보유하고 있다.

규칙은 **발간월이 아니라 보고서에서 추출한 보고대상기간에 건다.** 추출 실패 시 추정하지 말고 검토 큐로 보낸다. 예외 처리(다년도 추이 표, 보고기간 종료 후 활동, 연결/별도, 정정공시 접수번호, 통합보고서, 비12월 결산)는 「연계 검증 모듈 명세」 2.3절 참조.

2028년 이후 두 정보가 한 문서에 실리면 이 규칙은 불필요해진다.

**수집 메타데이터**: 회사 고유번호(DART 기준), 사업연도, 연결/별도 구분, 보고기간 시작·종료일.
...
**2-7. 연계 대조 (신설)**
- 하는 일: C군 발동 조건에 해당하는 클레임에 대해, 경로 B에서 조회한 재무 데이터와 대조
- **값 비교는 코드가, 차이 설명 문구의 탐색은 RAG가** 수행
- 산출: 4.10절의 `reconciliation` 객체. `status`는 `matched`/`needs_explanation`/`not_applicable` 셋뿐
- 차이가 있을 때 그 차이를 설명하는 문구를 보고서 전역에서 찾고, 찾으면 위치를 기록
  ...
  "reconciliation": [
    {
      "item": "organizational_boundary",
      "status": "needs_explanation",
      "sustainability_value": "국내 5개 사업장",
      "financial_value": "종속기업 12개사 연결",
      "difference_note": "운영통제 기준 vs 지배력 기준",
      "explanation_present": false,
      "explanation_location": null,
      "source_ref": {"system": "DART", "fy": 2025, "item": "종속기업 목록 주석", "fetched_at": "2026-09-14"},
      "basis": [{"standard": "IFRS S1", "clause": "21", "text": "재무제표와의 연계 정보 제공"}]
    }
  ],
...
│   └── iso_14021.yaml
├── accounting/                 # 연계 검증 (신설)
│   ├── ifrs_s1_connectivity.yaml   # 문단 21·B39~B44
│   ├── kifrs_1110.yaml             # 연결범위
│   ├── kifrs_1036_1037.yaml        # 손상·충당부채 (2단계)
│   ├── audit_standard_720.yaml     # 기타정보 검토 책임
│   └── reconciliation_rules.yaml   # C1~C5 대조 규칙·허용 차이 유형
├── assurance/
│   ├── isae3000.yaml
│   ├── aa1000as_v3.yaml
│   └── issa5000.yaml
...
연계 판정은 재현율보다 **정밀도가 결정적으로 중요하다.** 놓치는 것은 아쉬운 수준이지만, 정상적인 차이를 문제로 띄우면 기업 고객은 도구를 버린다.

| 지표 | 설명 |
|---|---|
| **연계 판정 정밀도** | `needs_explanation`으로 띄운 건 중 실제로 설명이 필요했던 비율. **주 지표** |
| 연계 판정 재현율 | 보조 지표 |
| **정상 차이 오탐률** | 기준 차이로 인한 정상적 불일치를 `needs_explanation`으로 잘못 띄운 비율. **목표: 5% 미만** |

**연계 전용 정답셋 (신설)**

기존 골드셋은 문장 내부 요소만 정답으로 갖고 있어 연계 판정을 평가할 수 없다. 별도 정답셋을 구축하며, **"정상적인 차이" 케이스를 반드시 포함**한다. 연결범위와 조직경계가 다른 것이 정상인 사례를 시스템이 오탐하지 않는지가 핵심 검증 포인트다.

- 목표 규모: 연계 항목별 30건 (C1~C4 합계 약 120건)
- 구성: 설명 있는 차이 / 설명 없는 차이 / 정상 차이(오탐 테스트) / 일치 — 네 유형 균등
...
**정보 취급 범위 확대에 따른 추가 요건 (신설)**

발간 전 지속가능경영보고서 원고에 더해 재무 정보까지 함께 다루면 미공개 중요정보 취급 범위가 넓어집니다. 특히 **결산 확정 전 재무 데이터**를 받는 경우의 처리 방침이 별도로 필요합니다. 공시된 DART 데이터만 사용하는 모드와 미공개 재무 데이터를 함께 받는 모드를 분리하고, 후자는 별도 계약과 접근 통제를 적용하십시오.

**책임 소재**를 계약과 제품 문구에 명시합니다.

- 이 도구는 **보증·인증이 아니며, 법적 판단을 대체하지 않습니다**
- **회계 처리의 적정성을 판단하지 않으며, 공시 간 차이에 대한 설명의 존재 여부만 점검합니다** — 연계 기능이 "충당부채가 과소계상되었다"는 회계 판단으로 오인되지 않도록 UI와 계약서에 명시
...
| 6. **연계 검증 (C1·C3)** | DART 조회 경로, 연결범위·정량 약속 대조, 연계 정답셋 | 정상 차이 오탐률 |
| 7. 제품화 | 검토 큐 UI, 리포트, 감사 로그, 대시보드 | 데모 |

**연계 검증은 C1(연결범위)·C3(정량 약속)만 1단계로 구현한다.** 이 둘이 실무에서 가장 자주 어긋나고, 판정 로직이 단순하며, 설명하기도 쉽다. C5(재무영향 정합)와 배출권 회계는 회계 도메인 지식 없이는 오탐이 쏟아지므로 2단계로 미룬다.
...
| DART | 금융감독원 전자공시시스템 |
| 연계(reconciliation) | 지속가능성 공시와 재무제표 간 정합 점검. IFRS S1 문단 21의 요구사항 |
| 연결범위 | 재무제표 작성 시 포함되는 종속기업의 범위. 배출량 조직경계와 다를 수 있음 |
| 주석(footnotes) | 재무제표 본문 숫자의 근거와 세부 내역. 연계 검증 정보의 대부분이 여기 있음 |
| 부문별 정보 | 사업 부문별 매출·이익 |
| 자본적지출(CAPEX) | 설비 등에 대한 투자 지출. 정량 투자 약속 대조에 사용 |
| 회계감사기준 720 | 감사인의 기타정보 검토 책임. 연계 기능의 수요 근거 |
| DART | 금융감독원 전자공시시스템. 재무 데이터를 구조화 형태로 조회 |
...
### v2.1 → v2.2 (2026-09-14) — 재무제표·사업보고서 연계 반영

| 영역 | 변경 |
|---|---|
| 1.1 검증 단위 | **"이 공시만" → "이 기업의 공시 패키지"**. IFRS S1 문단 21이 근거. 외부 제3자 데이터는 여전히 범위 밖 |
| 2.8 | 재무제표·주석·부문별 정보·CAPEX·회계감사기준 720 기초 지식 신설 |
| 3.1 / 3.7 | **연계·회계 계열 신설**. K-IFRS 1110·1036·1037, 감사기준 720 편입 |
| 4.5 | **C군(C1~C5) 루브릭 신설.** 전부 조건부, 기존 G·P·M 군과 분리 |
| 4.10 | **연계 판정 출력 규칙 신설.** 불일치는 결손이 아니며, 판정 대상은 "차이를 설명하는 문구가 있는가" |
| 6장 1-0 | **입력 경로 분리** (PDF 파싱 / DART API 조회), 대조 연도 결정 규칙 |
| 6장 2-7 | 연계 대조 단계 신설 |
| 8장 | `reconciliation` 객체, DART 메타데이터 필드 추가 |
| 8.4 | `config/accounting/` 신설 |
| 10.2 | 연계 전용 지표(정밀도 주 지표, 정상 차이 오탐률) + 연계 정답셋 |
| 11.4 | 미공개 재무정보 취급 방침, 회계 판단 오인 방지 문구 |
| 12.1 | 연계 검증 단계 추가. C1·C3만 1단계 |
