...
개발 중에는 변경 동작과 주요 호출 경로를 확인하는 관련 검사만 실행한다. 통합·릴리스 시점에는 lint, type check, unit, integration, contract, build, E2E, 보안 검증 중 해당 변경에 필요한 필수 검사를 실제로 실행하고 명령과 결과를 기록한다. 이미 통과한 검사는 관련 코드·환경 변경이나 새로운 실패 근거가 있을 때만 반복한다. 문서 수정만으로 앱 전체 테스트를 실행하지 않는다. 실제 모델·AWS 시험을 하지 못한 항목은 not_run으로 남긴다. 문서만으로 정확도 100%나 production 완료를 주장하지 않는다.
...
## 작업 방식 — 사용자 지시 2026-09-19
사용 가능한 `outcome-first` 스킬을 적용한다. 요청 범위는 유지하면서 전체 파이프라인에서 사용자 결과를 가장 많이 막는 문제부터 해결한다. 대표 입력으로 효과를 먼저 확인하고 기반 구현을 확장한다. 작은 오류별 전체 재검증, 겹치는 기록 문서, 효과가 입증되지 않은 구조 확장을 반복하지 않는다. 비용·권한·원문 근거 가드는 유지한다. 진행은 처리 결과·누락·오류·시간의 변화로 보고하며, 테스트 수를 모델 정확도로 표현하지 않는다. 사용자가 이번 작업 후 중단을 요청하면 후속 자동 목표 알림으로 재개하지 않는다.
